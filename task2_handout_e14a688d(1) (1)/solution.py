...
class SWAGInference(object):
    ...
    def __init__(
        ...
        if self.inference_mode == InferenceMode.SWAG_DIAGONAL:
            ...
            self.theta_square = self._create_weight_copy()
        ...
        if self.inference_mode == InferenceMode.SWAG_FULL:
            ...
            self.theta_swa = self._create_weight_copy()
            self.theta_square = self._create_weight_copy()
            self.theta_swa_running = self._create_weight_copy()
            self.theta_square_running = self._create_weight_copy()
    ...
    def update_swag(self,i) -> None:
        ...
        #    self.theta_swa[name] += param
        #    self.theta_square[name] += torch.square(param)
        ...
        if self.inference_mode == InferenceMode.SWAG_FULL:
            ...
            for name, param in current_params.items():
                #swag_update_step += 1
                #theta_swa_running_mean = self.theta_swa_running/self.bma_samples
                self.theta_swa[name] = (self.swag_update_step*self.theta_swa[name] + param)/(self.swag_update_step+1)
                self.theta_square[name] = (self.swag_update_step*self.theta_square[name] + torch.square(param))/(self.swag_update_step+1)
                #self.theta_swa_running[name] = self.theta_swa[name]/self.swag_update_step
                #self.theta_swa_running[name] = (self.swag_update_step*self.theta_swa_running[name]+param)/(self.swag_update_step+1)
                #self.theta_square_running[name] = (self.swag_update_step*self.theta_square_running[name] + torch.square(param))/(self.swag_update_step+1)
                self.D_column[name] = param - self.theta_swa[name]#/(self.swag_update_step+1)#self.theta_swa_running[name]
                #for name, param in current_params.items():
                #needed to add i to the parameters of the function it
                #self.D_column[name] = (param - (self.theta_swa[name]/ self.swag_update_step))
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            current_mean = self.theta_swa[name] #/ self.bma_samples
            current_mean_squared = torch.square(current_mean)
            current_theta_square = self.theta_square[name] #/ self.bma_samples
            ...
            if self.inference_mode == InferenceMode.SWAG_FULL:
                ...
                #current_var = (1/(self.deviation_matrix_max_rank-1))*torch.matmul(D_matrix,D_matrix)
                #current_var = torch.sqrt (torch.matmul(self.D_Hat))
                ...
                #sampled_param = current_mean + current_std * z_1 #+ (1/(self.deviation_matrix_max_rank-1))*self.D_column[name]*z_2
