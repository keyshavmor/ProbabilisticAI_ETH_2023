...
def log_posterior_probs(x):
    ...
    posterior_prob = np.empty((3,),dtype=np.float64)

    for i in range(3):
        posterior_prob[i] = np.log(HYPOTHESIS_SPACE[i].pdf(x)).sum() + np.log(PRIOR_PROBS[i])

    sum_post_prob = scipy.special.logsumexp(posterior_prob)
    log_p = posterior_prob - sum_post_prob
