...
class CalibrationNetwork(torch.nn.Module):
...
class SWAGInference(object):
    ...
    def __init__(
        ...
        if self.inference_mode == InferenceMode.SWAG_DIAGONAL:
            ...
            self.weight_swa = self._create_weight_copy()
            self.weight_swa_square = self._create_weight_copy()
        ...
        if self.inference_mode == InferenceMode.SWAG_FULL:
            ...
            self.weight_swa = self._create_weight_copy()
            self.weight_swa_square = self._create_weight_copy()
            self.weight_swa_running = self._create_weight_copy()
            self.D_Hat = collections.deque()
            self.D_column = self._create_weight_copy()
            self.D_rank = 0
            self.swag_update_step = 0
            self.loop_count = 0
    ...
    def expected_calibration_error(predictions, labels, num_bins=10):
        """
        Calculate Expected Calibration Error (ECE).

        Args:
            predictions (torch.Tensor): Predicted probabilities (float tensor).
            labels (torch.Tensor): True labels (long tensor).
            num_bins (int): Number of bins for histogram.

        Returns:
            float: Expected Calibration Error.
        """
        confidences, predictions_idx = torch.max(predictions, dim=1)
        accuracies = predictions_idx.eq(labels).float()
        ...
        for bin_start, bin_end in zip(bin_edges[:-1], bin_edges[1:]):
            # Filter predictions falling into the current bin
            in_bin = (confidences >= bin_start) & (confidences < bin_end)
            bin_confidences = confidences[in_bin]
            bin_accuracies = accuracies[in_bin]

            # Calculate the mean confidence and accuracy in the bin
            mean_confidence = bin_confidences.mean().item()
            mean_accuracy = bin_accuracies.mean().item()

            # Weighted difference (weighted by the number of samples in the bin)
            ece += abs(mean_confidence - mean_accuracy) * bin_confidences.size(0)

            # Normalize by the total number of samples
        ece /= predictions.size(0)

        return ece

    def update_swag(self,i) -> None:
        """
        ...
        if self.inference_mode == InferenceMode.SWAG_FULL:
            ...
            for name, param in current_params.items():
                self.weight_swa[name] += param
                self.weight_swa_square[name] += torch.square(param)
                self.weight_swa_running[name] = self.weight_swa[name]/self.swag_update_step
                self.D_column[name] = param - self.weight_swa_running[name]
 
            self.D_Hat.append(self.D_column)
            self.D_rank += 1
            if self.D_rank > self.deviation_matrix_max_rank:
                self.D_Hat.popleft()
    ...
    def calibrate(self, validation_data: torch.utils.data.Dataset) -> None:
        ...
        # Instantiate your calibration network
        self.calibration_network = CNN(in_channels=6,out_classes=6)

        # Define the loss function and optimizer for calibration
        self.calibration_criterion = torch.nn.BCELoss()
        self.calibration_optimizer = torch.optim.Adam(self.calibration_network.parameters(), lr=0.15)
        ...
        # Assuming your calibration model expects probabilities as input
        calibrated_predictions = self.calibration_network(self.network(val_xs))
        #Compute the calibration loss (you need to define your own calibration loss function)
        calibration_loss = self.calibration_criterion(calibrated_predictions, val_ys)        
    ...
    def predict_probabilities_swag(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        for _ in tqdm.trange(self.bma_samples, desc="Performing Bayesian model averaging"):
            ...
            model_sample_predictions = self.predict_probabilities_map(loader)
        ...
        bma_probabilities = torch.stack(per_model_sample_predictions, dim=2).mean(dim=2)
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            current_mean = self.weight_swa[name] / self.bma_samples
            current_mean_squared = torch.square(current_mean)
            current_theta_square = self.weight_swa_square[name] / self.bma_samples
            current_var = (current_theta_square - current_mean_squared)
            current_std = torch.sqrt((current_var))
            assert current_mean.size() == param.size() and current_std.size() == param.size()

            # Diagonal part
            sampled_param = current_mean + current_std * z_1 / np.sqrt(2)


            # Full SWAG part
            if self.inference_mode == InferenceMode.SWAG_FULL:
                z_2 = torch.randn(self.deviation_matrix_max_rank)
                size =list(param.size()) + [self.deviation_matrix_max_rank]
                D_matrix = torch.zeros(size)

                for i in range (self.deviation_matrix_max_rank):
                    D_matrix[..., i] = self.D_Hat[i][name]
                
                # TODO(2): Sample parameter values for full SWAG
                #raise NotImplementedError("Sample parameter for full SWAG")
                sampled_param += (D_matrix @ z_2) / np.sqrt(2*(self.deviation_matrix_max_rank-1))
