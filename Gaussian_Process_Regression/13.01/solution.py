...
class Model(object):
    ...
    def make_predictions(self, test_x_2D: np.ndarray, test_x_AREA: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        """
        test_x_tensor = torch.Tensor(test_x_2D)
        self.model.eval()
        self.likelihood.eval()
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            f_x = self.model(test_x_tensor)
            mean = f_x.mean.cpu().numpy()
            f_x_lower, f_x_upper = f_x.confidence_region()
            f_x_upper = f_x_upper.cpu().numpy()
            y_star = self.likelihood(f_x)
    ...
    def fitting_model(self, train_y: np.ndarray,train_x_2D: np.ndarray):
        ...
        train_X, val_X, train_Y, val_Y = train_test_split(train_x_2D,train_y, train_size=0.99,test_size=0.01,random_state=0,shuffle=True)
        ...
        optimizer = torch.optim.LBFGS(model.parameters(), lr=0.1, max_iter=100,history_size=10)
